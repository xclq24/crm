...
public class CustomerDaoImpl extends BaseDao<Customer> implements CustomerDao{
	...
	public void update(Customer customer) throws Exception {
		...
		update(
				"update customer set source=?,position=?,type=?,gender=?,email=?,phone=?,qq=?,addr=?,weibo=?,tel=?,msn=?,company=?,modifier=?,state=?,modifytime=? where id = ?",
				params);
		Customer c = findById(customer.getId());
		update("insert into allot (ename, cname) values(?, ?)", new Object[]{ customer.getEname(), c.getName()});
	}
